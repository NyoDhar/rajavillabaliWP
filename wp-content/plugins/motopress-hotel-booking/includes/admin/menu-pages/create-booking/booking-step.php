		...
		foreach ( $details as $number => $roomDetails ) {
			...
			if ( isset( $roomDetails['services'] ) && is_array( $roomDetails['services'] ) ) {
				foreach ( $roomDetails['services'] as $serviceDetails ) {
					...
					//BLK edit - allow all additional services for all accomodations
					...
					if ( $serviceId === false || $serviceAdults === false || $serviceAdults <= 0 || (isset($serviceDetails['quantity']) && $serviceQuantity < 1) ) {
						continue;
					}
					...
					$service = \MPHB\Entities\ReservedService::create( array(
						...
						'date_expected'	=> $serviceDetails['date_expected'] //BLK Edit - include date expected to data saved to booking
					) );
