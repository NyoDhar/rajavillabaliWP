...
class ReservedRoomsTemplater extends AbstractTemplater {
	...
	public function replaceTag( $match ){
		...
		switch ( $tag ) {
			...
			case 'cancel_policy':
				if ( isset( $this->reservedRoom ) ) {
					$roomType	 = MPHB()->getRoomTypeRepository()->findById( $this->reservedRoom->getRoomTypeId() );
					$cancel_policy = get_post_meta($roomType->getId(), 'cancel_policy', true);
					if(!empty($cancel_policy)){
						$the_policy = get_post($cancel_policy);
						$replaceText = 'Cancellation Policy: ';
						
						$cancel_policy_page = get_option('cancelation_policy_page');
						if(!empty($cancel_policy_page)){
							$policy_title = '<a href="'.get_page_link($cancel_policy_page).'">'.apply_filters('the_title', $the_policy->post_title).'</a>';
						}else{
							$policy_title = apply_filters('the_title', $the_policy->post_title);
						}
						
						$replaceText .= $policy_title;
					}
				}
				break;
		}
	...
	public function setupTags(){
		$tags = array(
			...
			array(
				'name'			 => 'cancel_policy',
				'description'	 => __( 'Cancellation Policy', 'motopress-hotel-booking' ),
			),
		);
