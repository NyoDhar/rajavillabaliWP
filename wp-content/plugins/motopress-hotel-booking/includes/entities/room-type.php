...
class RoomType {
	 ...
	 * @return float
	 */
	public function getDefaultPrice($check_discount = true){
		...
		if($check_discount){
			$discount = get_hot_deal_discount($this->originalId);
			
			if(!empty($discount)){
				$discount_value = $this->_defaultPrice * $discount / 100;
			}
			
			//$the_price = $this->_defaultPrice;
			//$this->_defaultPrice = $the_price - $discount_value;
		}
		
		return $this->_defaultPrice - $discount_value;
	}
	 ...
	 * @return float
	 */
	function getDefaultPriceForDates( \DateTime $checkInDate, \DateTime $checkOutDate, $check_discount=true, $property_id=null  ){
		$price = 0.0;
		...
		$accomodation_id = !empty($property_id) ? $property_id : $this->originalId;
		$rates = MPHB()->getRateRepository()->findAllActiveByRoomType( $accomodation_id, $dates );
		...
		if($check_discount){
			$discount = get_hot_deal_discount( $accomodation_id );
