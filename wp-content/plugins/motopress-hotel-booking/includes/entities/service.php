...
class Service {
	...
	public function getPriceWithConditions( $repeatability = true, $periodicity = true, $literalFree = true ){
		...
		//BLK edit
		$service_label = get_post_meta($this->originalId, 'service_label', true);
		...
		if ( !$this->isFree() ) {
			if ( $periodicity ) {
				...
				if ( $this->isPayPerNight() ) {
					//BLK edit
					$price .= $service_label['duration']; //__( 'Per Day', 'motopress-hotel-booking' );
				} else if ($this->isFlexiblePay()) {
                    $price .= $service_label['duration'];//__('Per Instance', 'motopress-hotel-booking');
                } else {
			...
			if ( $repeatability ) {
				...
				if ( $this->isPayPerAdult() ) {
					$price .= $service_label['qty'];__( 'Per Guest', 'motopress-hotel-booking' );
				} else {
