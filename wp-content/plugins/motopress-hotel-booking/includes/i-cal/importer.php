...
class Importer
{
    ...
    protected function isTooOld($event)
    {
        $today = date('Y-m-d');
        $tooOld = apply_filters('mphb_date_too_old_for_import', $today);

        return $event['checkOut'] < $tooOld;
    }

    protected function findIntersectingBookings($event)
    {
        return MPHB()->getBookingRepository()->findAll(array(
            'room_locked' => true,
    ...
    {
        $intersectionIds = array_map(function ($booking) {
            return $booking->getId();
        }, $intersectingBookings);

        $outdatedIds = array_map(function ($booking) {
            return $booking->getId();
        }, $outdatedBookings);
    ...
    public function import($event, $syncId, $queueId)
    {
		...
		//Force import the event, when date conflict found from different iCal Provider ( OTA )
		$conflictIds = $this->filterConflictingIds($intersectingBookings, $outdatedBookings);
		$wasCreated = $newId = $this->createBooking($event, $syncId, $queueId);
		if ($wasCreated) {
			$message = _n('Success. New booking #%d. - forced created. Dates from %2$s to %3$s are partially blocked by booking %4$s.', 'Success. New booking #%d. - forced created. Dates from %1$s to %2$s are partially blocked by bookings %3$s.', count($conflictIds), 'motopress-hotel-booking');
			$this->lastMessage = sprintf($message, $newId, $event['checkIn'], $event['checkOut'], '#' . implode(', #', $conflictIds));
			//$this->lastMessage = sprintf(__('Success. New booking #%d. - forced created ', 'motopress-hotel-booking'), $newId);
			return ImportStatus::SUCCESS;
		} else {
			//return ImportStatus::FAILED;
