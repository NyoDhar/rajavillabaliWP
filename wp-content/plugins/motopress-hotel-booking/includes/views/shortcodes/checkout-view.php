...
class CheckoutView {
	...
	public static function renderServiceChooser( $roomType, $roomIndex, $booking ){
		...
		//BLK edit - Show all additional services for all accomodations
		...
		<section id="mphb-services-details-<?php echo $roomIndex; ?>" class="mphb-services-details mphb-checkout-item-section">
			...
			<ul class="mphb_sc_checkout-services-list mphb_checkout-services-list">
				...
				<?php foreach ( $services as $index => $service ) { ?>
					...
					//BLK edit
					$service_label = get_post_meta($service->getOriginalId(), 'service_label', true);
					...
					<li>
						<label for="<?php echo $idPrefix; ?>-id" class="mphb-checkbox-label">
							...
							<a title="<?php _e('Read more details','motopress-hotel-booking'); ?>" href="<?php echo get_permalink( $service->getOriginalId() ); ?>" target="_blank"><?php echo $service->getTitle(); ?></a>
						...
						<div class="service-request-detail">
							<?php if ( $service->isPayPerAdult() && $roomType->getAdultsCapacity() > 1 ) { ?>
									<label for="<?php echo $idPrefix; ?>-date-expected">
										<?php _e('When do you expect to receive the service (date)?', 'motopress-hotel-booking') ?>
										<input type="text" class="rvb-datepicker" readonly name="<?php echo $namePrefix; ?>[date_expected]" id="<?php echo $idPrefix; ?>-date-expected">
									</label>
									
									<label for="<?php echo $idPrefix; ?>-adults">
										<span class="service-label"><?php _e( 'Quantity', 'motopress-hotel-booking' ); ?> :</span>
										...
										<?php echo !empty( $service_label['qty'] ) ? $service_label['qty'] : ''; ?>
									</label>
							...
							<?php if ($service->isFlexiblePay()) { ?>
								...
								?>
								<label>
									<span class="service-label"><?php _e('Duration', 'motopress-hotel-booking'); ?> :</span>
									<input type="number" name="<?php echo $namePrefix; ?>[quantity]" class="mphb_sc_checkout-service-quantity mphb_checkout-service-quantity" value="<?php echo esc_attr($minQuantity); ?>" min="<?php echo esc_attr($minQuantity); ?>" <?php echo !$service->isUnlimited() ? 'max="' . esc_attr($maxQuantity) . '"' : ''; ?> step="1" />
									<?php //_e('time(s) / day(s)', 'motopress-hotel-booking'); ?>
									<?php echo !empty( $service_label['duration'] ) ? $service_label['duration'] : ''; ?>
								</label>
							<?php } // Is flexible pay? ?>
						</div>
					</li>
