...
function mphb_tmpl_the_room_type_default_price( $id = null ){
	...
	$discount = get_hot_deal_discount($id);
	if(!empty($discount)){
		$price_before_discount = $roomType->getDefaultPrice(false);
		echo '<span class="price-before-discount">' . mphb_format_price($price_before_discount) . '</span>';
	}
 ...
 * @return string
 */
function mphb_tmpl_the_room_type_price_for_dates( \DateTime $checkInDate, \DateTime $checkOutDate, $property_id = null ){
	if(!empty($property_id)){
		$roomType = MPHB()->getRoomTypeRepository()->findById( $property_id );

		//$nights = MPHB()->getRulesChecker()->reservationRules()->getMinDaysAllSeason( $roomType->getOriginalId() );
		$price	 = $roomType->getDefaultPriceForDates($checkInDate, $checkOutDate, true, $property_id);
		$discount = get_hot_deal_discount($property_id);
		$price_before_discount = $roomType->getDefaultPriceForDates( $checkInDate, $checkOutDate, false, $property_id );
	}else{
		$price = MPHB()->getCurrentRoomType()->getDefaultPriceForDates( $checkInDate, $checkOutDate );
		$discount = get_hot_deal_discount($id);
		$price_before_discount = MPHB()->getCurrentRoomType()->getDefaultPriceForDates( $checkInDate, $checkOutDate, false );
	}
	...
	<form method="<?php echo esc_attr( $formMethod ); ?>" action="<?php echo esc_url( $actionUrl ); ?>" class="mphb-booking-form" id="<?php echo esc_attr( 'booking-form-' . $typeId ); ?>">
		...
		<?php if ( $isDirectBooking ) { ?>
			<div class="mphb-reserve-room-section mphb-hide">
				...
				<div class="row">
					<div class="col-sm-6">
						<input class="button" id="open-inquiry" type="button" value="<?php _e( 'Send Inquiry', 'rajavillabali' ); ?>" />
					</div>
					<div class="col-sm-6">
						<input class="button mphb-button mphb-confirm-reservation" type="submit" value="<?php _e( 'Book Now', 'rajavillabali' ); ?>" />
					</div>
				</div>
