jQuery(document).ready(function($){
	...
	blkddu.obj = jQuery("#media-uploader").dropzone({
		...
		success: function (file, response) {
			...
			if( typeof rvbparams !== 'undefined'){
				var obj = $(file.previewElement);
				obj.append('<div class="text-center">featured <input title="set as featured image" type="radio" name="featured-image" value="'+response+'"></div>');
	...
	blkddu.fileObj = jQuery("#file-uploader").dropzone({
		url: blkddu.upload,
		...
		success: function (file, response) {
			...
			file['attachment_id'] = response; // push the id for future reference
			//var ids = jQuery('#media-ids').val() + ',' + response;
			var img = '<input type="hidden" id="file-'+response+'" name="meta[rvb_property_legal_document][]" class="legal-docs-input" value="'+response+'">';
			jQuery('#files').append(img);
